...
import { setSessionThunk, getCurrentGameThunk } from '../store/thunks';

//  Determine if user is host (if so, show the start game button)
//  bring in TheCompetition
//  bring in Chat

const WaitingRoom = ({ setSession, getCurrentGame, game }) => {
  useEffect(() => {
  ...
  return (
    <div>
      ...
      <div>
        <button type="submit" className={game.host ? 'visible' : 'hidden'}>Start Game</button>
      </div>
    </div>
  );
};
